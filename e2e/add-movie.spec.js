...
test('buscar una película en TMDB y agregarla a la biblioteca', async ({ page }) => {
  ...
  const allTab = page.getByRole('button', { name: /^Todas/ })
  const countBefore = Number((await allTab.innerText()).replace(/\D/g, '') || '0')
  ...
  await expect(async () => {
    const countAfter = Number((await allTab.innerText()).replace(/\D/g, '') || '0')
